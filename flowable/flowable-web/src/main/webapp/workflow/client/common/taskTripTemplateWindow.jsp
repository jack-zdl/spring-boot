...
<script type="text/javascript">
    ...
    function completeTask(taskId,conditionExpression,buttonname) {
        ...
        var params;
        $.ajax({
            url:'/gateway/biz/tripartiteagreementTemplate/gettemplate',
            type: 'post',
            data:  {
                urid:"<%=urid%>"
            },
            ...
            success: function(text) {
                ...
                sign = SignBase64String(text, '<%=keynum%>', false);

                if(buttonname=="签约"){
                    params = {taskId: taskId, conditionExpression: conditionExpression,sign:sign};

                }else if(buttonname=="否决"){

                    sign = CheckBase64String('<%=keynum%>');

                    if(sign=="true"){
                        params = {taskId: taskId, conditionExpression: conditionExpression,sign:{}};
                    }else{
                        $('body').removeMask();
                        $.RMessage.alert("消息", "签名验签失败！");
                        return;
                    }
                }
